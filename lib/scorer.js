...
const NOT_GROUPS = new Set([
    ...
    // Surse si parti din ele (ray din blu-ray, dl din web-dl, etc)
...
function isValidGroupToken(token) {
    ...
    // Numere pure (ani, dimensiuni)
    if (/^\d+$/.test(token)) return false;
    // Numere cu punct (5.1, 7.1)
    if (/^[\d.]+$/.test(token)) return false;
...
function findMatchingGroup(videoText, subText) {
    ...
    for (const g of videoGroups) {
        if (subGroups.has(g)) {
            // Acceptam doar grupuri cunoscute SAU tokenuri lungi (4+ caractere)
            // "ray", "epic", "don" sunt prea scurte/comune ca sa fie sigure
