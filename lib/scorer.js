...
function calculateScore(subTitle, videoFilenameLower, signal, videoSourceTypeOverride = null, knownSeason = null, knownEpisode = null) {
    let score = 0;
    const breakdown = {};
    ...
    if (videoFilenameLower) {
        ...
        // 6. SEZON + EPISOD
        // Preferam sezonul/episodul cunoscut deja din ID-ul Stremio (sursa sigura,
        // vine din catalogul Stremio, nu poate fi gresit) — si nu doar ca fallback,
        // ci ca prioritate, pentru ca regex-ul slab \d+x\d+ (pt. formatul alternativ
        // "1x16") poate gasi fals-pozitive in placeholdere opace de la surse debrid
        // (ex: "TIJG5EPGMC5X4CDE" contine coincidental "5x4" -> ar fi dat S05E04!).
        // Cadem pe regex-ul din filename doar daca nu avem deloc sezon/episod cunoscut.
        ...
        if (seasonNum && episodeNum) {
            ...
            const subHasSeason = new RegExp(`s0?${seasonNum}(?!\\d)`, 'i').test(subTitleLower) ||
                                 new RegExp(`season\\s${seasonNum}(?!\\d)`, 'i').test(subTitleLower) ||
                                 new RegExp(`sezonul\\s${seasonNum}(?!\\d)`, 'i').test(subTitleLower);
            if (subHasFull) {
                score += 80;
                ...
                const otherSeasons = [
                    ...
                    ...(subTitleLower.match(/\bsezonul\s+\d{1,2}(?!\d)/gi) || []),
                    ...(subTitleLower.match(/\bseasons?\s+\d{1,2}(?!\d)/gi) || [])
                ];
                if (otherSeasons.length > 0) {
                    score -= 40;
                    breakdown.seEpisode = `alt sezon (nu S${season})(-40)`;
                }
            }
        }
