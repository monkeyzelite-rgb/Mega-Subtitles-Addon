...
// Tokenuri care NU sunt niciodata release groups, chiar daca apar dupa "-" sau in "[]"
const NOT_GROUPS = new Set([
    // Rezolutii
    '2160p', '1080p', '720p', '480p', '4k', '8k', '1080i', '576p',
    // Codecuri
    'x264', 'x265', 'h264', 'h265', 'hevc', 'avc', 'av1', 'xvid', 'divx', '10bit', '8bit',
    // Audio
    'dts', 'ac3', 'aac', 'dd', 'ddp', 'truehd', 'atmos', 'flac', 'mp3', 'eac3', 'dts-hd', 'dtshd',
    '5', '7', '1', '2', '51', '71', '20',
    // Surse
    'bluray', 'blu-ray', 'bdrip', 'brrip', 'remux', 'webrip', 'web-dl', 'webdl', 'web',
    'hdtv', 'dvdrip', 'dvdscr', 'hdrip', 'uhd', 'bd', 'hddvd', 'hdts', 'hd-ts',
    // Versiuni
    'extended', 'theatrical', 'unrated', 'uncut', 'redux', 'repack', 'proper', 'internal',
    'remastered', 'directors', 'final', 'ultimate', 'limited', 'hybrid',
    // Alte tag-uri
    'hdr', 'hdr10', 'dv', 'dovi', 'sdr', 'multi', 'dual', 'subs', 'sub', 'ro', 'en',
    'complete', 'season', 'part', 'cd1', 'cd2', 'mkv', 'mp4', 'avi', 'srt'
]);

function isValidGroupToken(token) {
    if (!token || token.length < 2) return false;
    // Numere pure (ani, dimensiuni) nu sunt grupuri
    if (/^\d+$/.test(token)) return false;
    // Numere cu punct (5.1, 7.1) nu sunt grupuri
    if (/^[\d.]+$/.test(token)) return false;
    // Tokenuri cunoscute ca nefiind grupuri
...
function extractReleaseGroups(text) {
    ...
    // Pattern 1: -GRUP la finalul unui token
...
function findMatchingGroup(videoText, subText) {
    ...
    for (const g of videoGroups) {
        if (subGroups.has(g)) {
            if (GROUPS.includes(g) || g.length >= 3) return g;
        }
    }
    return null;
}
