...
// Interval de episoade fara sezon lipit de el: "Episoadele 1-22", "Episodes 1 to 10".
// (Cu sezonul alaturi, "Sezonul 2 episoadele 1-10", il citeste deja WORDS_RE.)
function parseBareEpisodeRanges(text) {
    const t = (text || '').toLowerCase();
    const ranges = [];
    for (const m of t.matchAll(/(?<![a-z])(?:episoadele|episoade|episodes)[\s._:-]*(\d{1,3})[\s._]*(?:-|–|—|pana la|până la|to)[\s._]*(\d{1,3})(?![0-9a-z])/g)) {
        const from = parseInt(m[1], 10), to = parseInt(m[2], 10);
...
function calculateScore(subTitle, videoFilenameLower, signal, videoSourceTypeOverride = null, knownSeason = null, knownEpisode = null, videoSourceTypeIsGuess = false, ctx = {}) {
    ...
    if (seasonNum && episodeNum) {
        ...
        // "Episoadele 1-22" cand vrem E23: intervalul incepe de la 1 si se termina
        // inainte de episodul cerut — pachetul nu-l contine. Doar intervale care
        // incep cu 1: "Episoadele 20-30" poate fi numerotare absoluta (anime),
        // unde E03 din sezonul 2 chiar e episodul 25.
        const bareEpRanges = parseBareEpisodeRanges(subTitleLower);
        const episodeBeyondRange = bareEpRanges.length > 0 && episodeNum < 100 &&
            bareEpRanges.every(r => r.from === 1 && episodeNum > r.to);
        ...
        if (subHasFull) {
        ...
        } else if (episodeBeyondRange && refsForSeason.length === 0 && !allRefs.ambiguous) {
            score -= 120;
            breakdown.seEpisode = `episodul ${episodeNum} nu e in intervalul de episoade — exclus(-120)`;
            breakdown.wrongEpisode = true;
            ...
            // Un interval explicit de sezoane care NU contine sezonul cerut
            // ("Sezoanele 1-5" cand vrem S06) e tot alt sezon, sigur.
            if (otherSeasons.length > 0 || seasonRanges.length > 0) {
...
module.exports = { calculateScore, getSourceType, detectFramerate, detectVersions, hasKeyword, parseEpisodeRefs, parseSeasonRanges, parseBareEpisodeRanges, extractYears, normalizeSeasonText };
