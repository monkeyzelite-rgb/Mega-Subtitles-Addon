...
const { extractYears } = require('./scorer');

const API_URL = 'https://api.regielive.ro/bazarr/search.php';
// Aceeasi variabila ca in server.js (descarcare) — pana acum cautarea folosea
// mereu cheia comuna, chiar daca REGIELIVE_API_KEY era setata (INSTALL.md
// promite ca variabila e suficienta, fara editare de cod).
const API_KEY = process.env.REGIELIVE_API_KEY || 'API-BAZARR-YTZ-SL';
// Fara timeout, o cerere blocata tinea ocupat unul din cele 2 sloturi de
// conexiune si tot raspunsul catre Stremio (addon.js asteapta toate sursele).
...
async function getCinemetaInfo(imdbId, type) {
    try {
        ...
        const res = await axios.get(`https://v3-cinemeta.strem.io/meta/${type}/${baseId}.json`, { timeout: REQUEST_TIMEOUT_MS });
...
// getMeta (optional): functie care intoarce meta-ul Cinemeta deja cerut de
// addon.js pt. celelalte surse — evita a doua cerere Cinemeta identica.
async function searchRegieLive(imdbId, type, videoFilename, getMeta = null) {
    ...
    const searchPromise = _searchRegieLive(imdbId, type, videoFilename, errorTracker, getMeta);
...
async function _searchRegieLive(imdbId, type, videoFilename, errorTracker, getMeta = null) {
    console.log(`\n--- [CĂUTARE NOUĂ] ---`);
    ...
    function ingestResponse(response, { isFallbackByName, referenceTitle, keepSub = null }) {
        ...
        for (const filmKey in filme) {
            ...
            for (const subKey in subs) {
                ...
                if (keepSub && !keepSub(subs[subKey].titlu || '')) {
                    console.log(`[FILTRU AN] Ignor "${subs[subKey].titlu}" (alt an decat cel cautat)`);
                    continue;
                }
    ...
    if (subtitles.length < MIN_RESULTS_THRESHOLD) {
        // Cererea proprie ramane doar ca rezerva, daca cea comuna a esuat.
        meta = (getMeta ? await getMeta().catch(() => null) : null) || await getCinemetaInfo(imdbId, type);
        if (meta) {
            const textParams = {};
            if (type === 'series') {
    ...
    // --- METODA 3: doar Nume — DOAR daca Metoda 2 (cu an) n-a gasit nimic ---
    // Raspunsul RegieLive contine doar numele filmului ("film") si subtitrarile,
    // fara an, deci cautarea fara an amesteca filme cu acelasi nume. Confirmat pe
    // productie: pt. "Mayday" (2026) aducea "Mayday 2021", "Mayday 2019" si
    // episoade din serialul "Mayday" (1x01...), iar "Mayday 2021" a fost ALEASA
    // AUTOMAT. Acum ruleaza doar ca plasa de siguranta (anul de pe RegieLive
    // difera de cel din Cinemeta) si pastreaza doar subtitrarile cu anul corect
    // (+-1) in titlu; la seriale, doar pe cele fara un alt an explicit (numele
    // de episod rareori contin anul). Fara an cunoscut, Metoda 2 a fost deja o
    // cautare doar dupa nume — nu o repetam.
    if (subtitles.length === 0 && wantedYear) {
        const yearOk = (y) => Math.abs(y - wantedYear) <= 1;
        const keepSub = (title) => {
            const years = extractYears(title, meta.name);
            return type === 'series' ? (years.length === 0 || years.some(yearOk)) : years.some(yearOk);
        };
        if (meta) {
            ...
            try {
                ...
                ingestResponse(response, { isFallbackByName: true, referenceTitle: meta.name, keepSub });
            } catch (err3) {
