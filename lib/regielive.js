...
// persistent (local/Raspberry Pi), dar inutil pe Vercel, unde cereri concurente
// pot ajunge pe instante serverless separate, fiecare cu propria memorie. Fara
// un numarator PARTAJAT, limita reala de 8/minut a RegieLive nu s-ar mai
// respecta corect, cu risc de ban. Activa doar cand exista Redis configurat.
const USE_DISTRIBUTED_RATE_LIMIT = !!process.env.UPSTASH_REDIS_REST_URL;
...
async function acquireApiSlotDistributed() {
    ...
    while (true) {
        ...
        if (!minute.success) {
            await new Promise(r => setTimeout(r, Math.max(200, minute.reset - Date.now())));
            continue;
        }
        const second = await distributedLimiters.perSecond.limit('global');
        if (!second.success) {
            await new Promise(r => setTimeout(r, Math.max(200, second.reset - Date.now())));
            continue;
        }
        return;
    }
